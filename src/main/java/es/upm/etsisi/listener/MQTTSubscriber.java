...
import es.upm.etsisi.restservice.SimpleDatabase;
...
public class MQTTSubscriber implements IListener<String, String>, MqttCallback {
	...
	public MQTTSubscriber(ConfigurableEnvironment environment)
		...
		this.brokerUrl = environment.getProperty("mqtt_url", "tcp://localhost:2883");
	...
	public void messageArrived(String topic, MqttMessage message) throws Exception {
		...
		String room = "office";
		if (msg.get("id").asText().equals("sensor_dining_room")) {
			room = "dining";
		}
		
		if (msg.get("datatype").asText().equals("emergency") && msg.get("value").asBoolean()) {
			isEmergency = true;
			SimpleDatabase.setEmergency(data);
		}
		else if (msg.get("datatype").asText().equals("emergency") && !msg.get("value").asBoolean()) {
			isEmergency = false;
		}
		
		if (!isEmergency) {
			switch (msg.get("datatype").asText()) {
				case "emergency": SimpleDatabase.setEmergency(data); break;
				case "luminosity": 
					if (room.equals("office")) {
						SimpleDatabase.setOfLuminosity(data);
					}
					else {
						SimpleDatabase.setDrLuminosity(data);
					}
					break;
				case "humidity":
					if (room.equals("office")) {
						SimpleDatabase.setOfHumidity(data);
					}
					else {
						SimpleDatabase.setDrHumidity(data);
					}
					
					break;
				case "temperature": 
					if (room.equals("office")) {
						SimpleDatabase.setOfTemperature(data);
					}
					else {
						SimpleDatabase.setDrTemperature(data);
					}
